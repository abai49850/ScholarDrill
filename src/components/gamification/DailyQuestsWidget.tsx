...
import type { DailyQuest } from "@/lib/statsApi";
...
export const DailyQuestsWidget = ({ quests, claimedIds, onClaim }: Props) => {
  return (
        ...
        {quests.map((quest) => {
          ...
          return (
            <div key={quest.id} className={`p-4 rounded-2xl border-2 relative overflow-hidden transition-all ${
              isClaimed ? "bg-muted/30 border-border/50 opacity-70" :
              ...
              <div className="flex justify-between gap-4">
                <div className="flex-1">
                  <h4 className={`font-bold text-sm ${isComplete && !isClaimed ? "text-success" : ""}`}>{quest.title}</h4>
                ...
                <div className="shrink-0 flex flex-col items-center justify-center border-l-2 border-border/50 pl-4">
                  ...
                  {isComplete && !isClaimed ? (
                    <Button
                      ...
                      className="h-7 text-xs px-3 rounded-lg bg-success hover:bg-success/90 shadow-md transform hover:-translate-y-0.5 transition-transform"
                    >
                      Claim
                    </Button>
                  ) : (
